...
class HSSegmentationModelOutput : MLFeatureProvider {
    ...
    lazy var segmentation_image_output: MLMultiArray = {
        [unowned self] in return self.provider.featureValue(for: "segmentation_image_output")!.multiArrayValue
        }()!
    ...
    init(segmentation_image_output: MLMultiArray) {
        self.provider = try! MLDictionaryFeatureProvider(dictionary: ["segmentation_image_output" : MLFeatureValue(multiArray: segmentation_image_output)])
    }
